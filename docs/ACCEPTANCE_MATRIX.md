...
| 备份恢复 | 87% | 本地备份、恢复计划、定时备份、Bucket 为空、Object Storage 边界提示全部稳定 | Web + 脚本 + 测试备份包 | 进行中 |
| 救援中心 | 68% | 当前定义为安全向导、脚本入口、风险提示 100%；高危自动救援继续保留实验区 | Web 页面 + 文档边界 + 专用机型测试 | 进行中 |
| CI/测试 | 72% | 发布前能发现构建、脚本换行、脚本语法、API 映射、生产包资产、路由验收遗漏 | `verify-release.sh` + `acceptance-check.mjs` | 进行中 |
...
| `scripts/remote-smoke-test.sh` | 无 Node 环境可跑，17 项接口验收 | 已验收一期 |
| `scripts/remote-smoke-test.mjs` | Windows/Node 环境可跑，curl 回退 | 已验收一期 |
